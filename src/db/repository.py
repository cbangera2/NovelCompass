        ...
        with self.conn:
            ...
            cur.execute("""
                ...
                ON CONFLICT(id) DO UPDATE SET
                    slug=COALESCE(excluded.slug, novels.slug),
                    title=excluded.title,
                    associated_names=CASE WHEN excluded.associated_names='[]' THEN novels.associated_names ELSE excluded.associated_names END,
                    author=CASE WHEN excluded.author IS NULL OR excluded.author='Unknown' THEN novels.author ELSE excluded.author END,
                    language=COALESCE(excluded.language, novels.language),
                    synopsis=CASE WHEN excluded.synopsis IS NULL OR excluded.synopsis='' THEN novels.synopsis ELSE excluded.synopsis END,
                    rating=CASE WHEN excluded.rating IS NULL OR excluded.rating=0 THEN novels.rating ELSE excluded.rating END,
                    rating_votes=CASE WHEN excluded.rating_votes IS NULL OR excluded.rating_votes=0 THEN novels.rating_votes ELSE excluded.rating_votes END,
                    rating_votes_5=CASE WHEN excluded.rating_votes_5 IS NULL OR excluded.rating_votes_5=0 THEN novels.rating_votes_5 ELSE excluded.rating_votes_5 END,
                    rating_votes_4=CASE WHEN excluded.rating_votes_4 IS NULL OR excluded.rating_votes_4=0 THEN novels.rating_votes_4 ELSE excluded.rating_votes_4 END,
                    rating_votes_3=CASE WHEN excluded.rating_votes_3 IS NULL OR excluded.rating_votes_3=0 THEN novels.rating_votes_3 ELSE excluded.rating_votes_3 END,
                    rating_votes_2=CASE WHEN excluded.rating_votes_2 IS NULL OR excluded.rating_votes_2=0 THEN novels.rating_votes_2 ELSE excluded.rating_votes_2 END,
                    rating_votes_1=CASE WHEN excluded.rating_votes_1 IS NULL OR excluded.rating_votes_1=0 THEN novels.rating_votes_1 ELSE excluded.rating_votes_1 END,
                    reading_list_count=CASE WHEN excluded.reading_list_count IS NULL OR excluded.reading_list_count=0 THEN novels.reading_list_count ELSE excluded.reading_list_count END,
                    chapters_orig=CASE WHEN excluded.chapters_orig IS NULL OR excluded.chapters_orig=0 THEN novels.chapters_orig ELSE excluded.chapters_orig END,
                    chapters_trans=CASE WHEN excluded.chapters_trans IS NULL OR excluded.chapters_trans=0 THEN novels.chapters_trans ELSE excluded.chapters_trans END,
                    status_trans=COALESCE(excluded.status_trans, novels.status_trans),
                    year=COALESCE(excluded.year, novels.year),
                    cover_url=COALESCE(excluded.cover_url, novels.cover_url),

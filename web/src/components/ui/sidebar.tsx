...
export function SidebarProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const keyboard = (event: KeyboardEvent) => {
      ...
      if (
        ...
        (event.altKey && event.code === 'Backslash')
      ) {
  ...
  return (
    <SidebarContext.Provider value={{ collapsed, mobileOpen, setMobileOpen, toggleSidebar }}>
      <div className="sidebar-provider" data-collapsible={collapsed ? 'icon' : 'expanded'}>
        {children}
      </div>
...
export function Sidebar({ children }: { children: ReactNode }) {
  ...
  return (
    <>
      <aside className="sidebar-root" aria-label="Primary">
        {children}
      </aside>
      <Dialog.Root open={mobileOpen} onOpenChange={setMobileOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Popup className="sidebar-mobile-sheet" aria-label="Navigation">
            <Dialog.Close className="sidebar-mobile-close" aria-label="Close navigation">
              <X size={18} />
            </Dialog.Close>
            {children}
          </Dialog.Popup>
...
export function SidebarHeader(props: HTMLAttributes<HTMLDivElement>) {
  return <div {...props} className={`sidebar-header ${props.className || ''}`} />;
}
export function SidebarContent(props: HTMLAttributes<HTMLDivElement>) {
  return <div {...props} className={`sidebar-content ${props.className || ''}`} />;
}
export function SidebarFooter(props: HTMLAttributes<HTMLDivElement>) {
  return <div {...props} className={`sidebar-footer ${props.className || ''}`} />;
}
export function SidebarGroup(props: HTMLAttributes<HTMLDivElement>) {
  return <div {...props} className={`sidebar-group ${props.className || ''}`} />;
}
export function SidebarGroupLabel(props: HTMLAttributes<HTMLDivElement>) {
  return <div {...props} className={`sidebar-group-label ${props.className || ''}`} />;
}
export function SidebarMenu(props: HTMLAttributes<HTMLUListElement>) {
  return <ul {...props} className={`sidebar-menu ${props.className || ''}`} />;
}
export function SidebarMenuItem(props: HTMLAttributes<HTMLLIElement>) {
  return <li {...props} className={`sidebar-menu-item ${props.className || ''}`} />;
}
  ...
  if (asChild) {
    ...
    if (isValidElement(child))
      return cloneElement(child as ReactElement<any>, {
        ...common,
        ...child.props,
        className: `${common.className} ${child.props.className || ''}`,
      });
  }
  return (
